                ...
                switch (piece.getType()) {
                    case PAWN -> {
                        ...
                        if (naturalMove != Square.INVALID && board.isEmpty(naturalMove)) {
                            Move natural = new Move(p, location, naturalMove, state.getDiceRoll(), state.getColor());
                            ...
                            if (p.canPromote(naturalMove)) {
                                ...
                                if (state.getDiceRoll() != 1 && state.getDiceRoll() != 6) {
                                    ...
                                    natural.promotionPiece = Piece.QUEEN.getColoredPiece(state.getColor());
                                }
                            ...
                            if (doubleJump != Square.INVALID && board.isEmpty(doubleJump) && piece.canDoubleJump(location))
                                validMoves.add(new Move(p, location, doubleJump, state.getDiceRoll(), state.getColor()));
                        }

                        for (int k = 1; k < 3; k++) {
                            Square captureTarget = Square.getSquare(location.getSquareNumber() + piece.getOffsets()[k]);
                            if (captureTarget != Square.INVALID && board.getPieceAt(captureTarget) != EMPTY && !board.getPieceAt(captureTarget).isFriendly(state.getColor())) {
                                Move capture = new Move(p, location, captureTarget, state.getDiceRoll(), state.getColor());
                                ...
                                if (p.canPromote(captureTarget)) {
                                    ...
                                    if (state.getDiceRoll() != 1 && state.getDiceRoll() != 6) {
                                        ...
                                        //auto promote to Queen
                                        //TODO: potentially give AI the choice to promote to knight in cases where it would lead to a king capture in the next turn
                                        capture.promotionPiece = Piece.QUEEN.getColoredPiece(state.getColor());
                                    }
                    ...
                    case KNIGHT -> {
                        for (int offset : piece.getOffsets()) {
                            ...
                            if (target != Square.INVALID) {
                                if (board.isEmpty(target) || !board.getPieceAt(target).isFriendly(state.getColor()))
                                    validMoves.add(new Move(p, location, target, state.getDiceRoll(), state.getColor()));
                            }
                        }
                    }
                        ...
                        if (piece.getColor() == Side.WHITE) {
                            if (location.getSquareNumber() == 4) {
                                ...
                                if (board.isEmpty(location.getSquareRight()) && board.isEmpty(getSquare(6)) && state.isShortCastlingWhite()) {
                                    validMoves.add(new Move(p, location, getSquare(6), state.getDiceRoll(), state.getColor()));
                                ...
                                if (board.isEmpty(location.getSquareLeft()) && board.isEmpty(getSquare(2)) && board.isEmpty(getSquare(1)) && state.isLongCastlingWhite()) {
                                    validMoves.add(new Move(p, location, getSquare(2), state.getDiceRoll(), state.getColor()));
                                    //state.setApplyCastling(true);
                                }
                            }
                        }
                            ...
                            if (location.getSquareNumber() == 116) {
                                ...
                                if (board.isEmpty(location.getSquareRight()) && board.isEmpty(getSquare(118)) && state.isShortCastlingBlack()) {
                                    validMoves.add(new Move(p, location, getSquare(118), state.getDiceRoll(), state.getColor()));
                                    //state.setApplyCastling(true);
                                }
                                //LONG BLACK
                                if (board.isEmpty(location.getSquareLeft()) && board.isEmpty(getSquare(114)) && board.isEmpty(getSquare(113)) && state.isLongCastlingBlack()) {
                                    validMoves.add(new Move(p, location, getSquare(114), state.getDiceRoll(), state.getColor()));
                                    //state.setApplyCastling(true);
                                }
                            }
                        }
                    ...
                    case BISHOP, ROOK, QUEEN -> {
                        for (int offset : piece.getOffsets()) {
                            ...
                            if (target != INVALID && !board.getPieceAt(target).isFriendly(state.getColor())) {
                                validMoves.add(new Move(p, location, target, state.getDiceRoll(), state.getColor()));
                            }
                        }
                    }
